...
export function parseOfxBytes(buffer, filename) {
  ...
  for (const block of blocks) {
    ...
    if (amount >= 0) {
      ...
      records.push({ entrada: formatDecimal(amount), item: cleanText(item), saida: "", data, banco });
    } else {
      totalSaidas += Math.abs(amount);
      records.push({ entrada: "", item: cleanText(item), saida: formatDecimal(Math.abs(amount)), data, banco });
    }
  }
